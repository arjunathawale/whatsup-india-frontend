...
const AddList = ({ botId, isListOpen, setIsListOpen, data = {}, setCurrentScript, redirectData, getScript }) => {
  ...
  const [formData, setFormData] = useState({
    id: data?._id,
    botId: botId,
    messageType: data?.messageType || "LIST",
    ...
    expectedMessage: data?.expectedMessage || "list_reply",
  ...
  const addButton = () => {
    if (formData.buttonOrListData.length <= 10) {
      setFormData({
        ...formData,
      ...
      showToast('info', 'You can add only 10 buttons');
    }
  }
    ...
    <div>
      ...
      <div
        className={`
              ...
              ${isListOpen ? "opacity-100 scale-100" : "opacity-0 scale-90 pointer-events-none"
          }`}
      >
        ...
        <div className="bg-white w-full max-w-lg mx-4 sm:mx-auto rounded-lg shadow-lg p-6 overflow-y-auto h-[90%]">
          ...
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-bold">{Object.keys(data).length ? "Update List Script" : "Add List Script"}</h2>
            <button
            ...
            <div className='col-span-full' >
              {
                formData.buttonOrListData.map((item, index) => (
                  <div className='bg-gray-300 p-2 rounded-md' key={index}>
                    <div className='grid grid-cols-1 md:grid-cols-2 gap-2 p-2 rounded-sm bg-gray-100' >
                      <div className='' >
                      ...
                      <div className="">
                        ...
                        <div className="relative">
                          <select
                            className="w-full text-sm border rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 outline-none"
                            value={formData?.buttonOrListData[index]?.id}
                            onChange={(e) => handleChangeRedirectId(index, "id", e.target.value)}
                          >
                      ...
                      <div className='col-span-full' >
                        ...
                        <input
                          ...
                          maxLength={60}
                          className="w-full text-sm border rounded px-3 py-2 focus:ring-1 focus:ring-blue-500 outline-none"
                          value={formData?.buttonOrListData[index]?.description}
                          onChange={(e) => handleChangeRedirectId(index, "description", e.target.value)}
                        />
                        <p className='text-[9px] text-red-600 font-semibold ml-1 flex justify-between items-center'>
                          {formData?.buttonOrListData[index]?.optionName.length}/60
                        </p>
                      </div>
                    </div>
                  </div>
              ...
              <div className='flex justify-end'>
                <IoAddCircleOutline
                  className={`text-blue-500 h-6 w-6
                              ${formData.buttonOrListData.length < 10 ?
          ...
          >
            {Object.keys(data).length ? "Update List Script" : "Add List Script"}
          </button>
        </div>
